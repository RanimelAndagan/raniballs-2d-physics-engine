...
/* =====================================================================
   ...
   the core idea, same as basically any game loop:
       every frame (~60x a second)
         1. move things a bit
         2. draw them
         3. ask the browser to run it again
   ===================================================================== */
/* ---------------------------------------------------------------------
   2. SETTINGS TO PLAY WITH
   ---------------------------------------------------------------------
   change these and reload to see the physics change.
--------------------------------------------------------------------- */
...
/* ---------------------------------------------------------------------
   3. THE CANVAS
   ---------------------------------------------------------------------
   canvas is just a rectangle of pixels i can draw on. ctx is the
   paintbrush, everything goes through it.
--------------------------------------------------------------------- */
...
class Ball {
    ...
    constructor(x, y, vx = 0, vy = 0) {
        ...
        this.color = pick(COLORS); // the base color, never changes
        this.flash = 0; // how lit up it is now (0 = normal, 1 = white). starts un-flashed
    }
...
--------------------------------------------------------------------- */
let pressPos = null; // where mouse went down (null = not pressing)
let dragPos = null; // where the mouse is mid-drag
let mouse = { x: -9999, y: -9999 }; // live cursor pos, starts off-screen
...
window.addEventListener("keydown", (e) => {
    unlockAudio();
    if (e.repeat)
        return; // ignore the auto-fire while a key is held, so one press = one quake
    const key = e.key.toLowerCase();
    if (key === "c")
        balls.length = 0; // C  wipes everything
    if (key === "e")
        triggerQuake(); // E  shakes the place up
    if (key === "w") { // W  toggle wind on/off
        windOn = !windOn;
        console.log("WIND:", windOn ? "on" : "off");
    }
    if (key === "f") { // F  flip wind direction
        windDir *= -1;
        console.log("WIND direction:", windDir > 0 ? "right" : "left");
    }
    if (key === "g") { // G  flip gravity up/down
        gravityFlipped = !gravityFlipped;
        console.log("GRAVITY:", gravityFlipped ? "up" : "down");
    }
    if (key === "h") { // H  toggle black hole at cursor
        blackHoleOn = !blackHoleOn;
        console.log("BLACK HOLE:", blackHoleOn ? "on" : "off");
    }
    if (key === "x")
        triggerExplosion(mouse.x, mouse.y); // X  explosion at the cursor
...
function applyBlackHole() {
    ...
    for (const ball of balls) {
        ...
        const dist = Math.hypot(dx, dy);
        if (dist === 0 || dist >= BLACK_HOLE_REACH)
            continue;
...
function triggerExplosion(cx, cy) {
    for (const ball of balls) {
        ...
        if (dist >= EXPLOSION_RADIUS)
            continue; // too far away to feel it
...
function playClick(volume) {
    if (!audioCtx)
        return; // audio not unlocked yet, stay silent
    if (activeVoices >= MAX_VOICES)
        return; // too many at once, drop this one
    if (volume <= 0)
        return; // nothing to hear
...
console.log("RANIBALLS controls:\n" +
